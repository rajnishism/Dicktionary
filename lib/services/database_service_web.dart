...
/// Web-specific database service using sqflite_common_ffi_web.
...
class DatabaseService {
  ...
  Future<Database> _initDatabase() async {
    ...
    final path = 'vocabulary_memory.db';
  ...
  Future<WordMemory> insertOrUpdateWordWithQueue(String word, String meaning) async {
    ...
    if (existing.isNotEmpty) {
      ...
      updatedMap['queue_position'] = maxPos + 1;
  ...
  Future<List<WordMemory>> getPersonalQueue({int limit = 20}) async {
    final db = await database;
    final maps = await db.query('word_memory', orderBy: 'queue_position ASC', limit: limit);
